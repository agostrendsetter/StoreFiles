        ...
        callMainnetNetwork('',addr, contractAddr, value, 3)
            .then(function (data) {
                if (data.error) {
                    ...
                    getZARPrice(data.symbol, 'ZAR')
                        .then(function (zarPrice) {                          
                            generateTokenInfo(data, value, zarPrice, contractAddr);
                        });                   
                }
...
function generateTokenInfo(data, network, zarPrice, contractAddr) {
    ...
    try {
        ...
        if (data.result == null) {
            ...
            var value = zarPrice.ZAR ? new BigNumber(balance * zarPrice.ZAR) + ' ZAR  <font size="1">(@' + zarPrice.ZAR + '/ZAR)</font>' : '0.00000000';
